...
class AnimeGridView extends StatefulWidget {
  ...
  AnimeGridView({Key key, this.anime, this.link}) : super(key: key);

  @override
  AnimeGridState createState() => AnimeGridState(anime, link);
}

class AnimeGridState extends State<AnimeGridView> {
  bool _isPerforming = false;
  int pageNumber = 2;
  final List<Animes> anime;
  final String link;

  AnimeGridState(this.anime, this.link);
  ...
  void loadMore(int idx) async {
    if (!_isPerforming) {
      setState(() {
        _isPerforming = true;
      });
      List<Animes> animenext =
          await Animes.loadFromUrl(link + '&page=' + idx.toString());
      setState(() {
        anime.addAll(animenext);
        pageNumber++;
        _isPerforming = false;
      });
    }
